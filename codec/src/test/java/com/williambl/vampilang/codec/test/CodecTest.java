...
public class CodecTest {
    ...
    private static class VTypeCodecRegistryImpl implements VTypeCodecRegistry {
        ...
        public Codec<VExpression> expressionCodecForType(VType type) {
            return Codec.either(
                    new ValueCodec(type, this), //TODO spec
                            this.functionCodec.comapFlatMap(f -> type.contains(((VExpression.FunctionApplication) f.resolveTypes(new EvaluationContext.Spec())).resolvedSignature().outputType()) ? DataResult.success(f) : DataResult.error(() -> "Unmatched type"),
                                    Function.identity()))
